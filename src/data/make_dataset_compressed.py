...
from src.utils import get_padding, get_device
...
class DatasetVesuviusCompressed(Dataset):
    ...
    def make_image(self, fragment_path, shape, padding):
        ...
        image_compressed = np.load(os.path.join(fragment_path, 'fragment.npz'))['image']
        for i, slice in tqdm(enumerate(self.slices), total=len(self.slices)):
            image[i, ...] = image_compressed[slice, ...]
